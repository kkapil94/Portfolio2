          ...
          <div className="grid xs:max-md:grid-cols-1 md:max-lg:grid-cols-2 justify-items-center grid-cols-3 xs:max-md:gap-28 gap-4 md:max-lg:[&>*:last-child]:mt-32">
            {connect.map(item=>(
              ...
              <a href={item.href} target="_blank">
                ...
                <img className="absolute xs:max-sm:w-44 sm:max-md:w-52 md:max-4xl:w-60 xs:max-sm:-translate-y-24 sm:max-md:-translate-y-28 md:max-4xl:-translate-y-32" src={item.img_src} alt="" />
              ...
              <div className="xs:max-md:px-4 px-12 xs:max-sm:mt-20 sm:max-md:mt-28 md:max-4xl:mt-36">
                <h4 className="xs:max-md:text-xl text-2xl text-white xs:max-md:mb-0 ">{item.name}</h4>
                <p className="xs:max-md:text-lg text-xl text-slate-400 ">{item.desc}</p>
                <a className="my-6 text-md rounded-2xl px-4 py-1 bg-white font-md inline-block" href={item.href} target="_blank">Follow on {item.name}</a>
              </div>
            </div>
            ))
              }
